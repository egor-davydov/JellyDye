...
namespace CrazyGames.TreeLib
{
  ...
  {
    public static void TreeToList<T>(T root, IList<T> result)
      where T : TreeElement
    {
      if (result == null)
    ...
    public static T ListToTree<T>(IList<T> list)
      where T : TreeElement
    {
      // Validate input
    ...
    public static void ValidateDepthValues<T>(IList<T> list)
      where T : TreeElement
    {
      if (list.Count == 0)
      ...
      for (int i = 1; i < list.Count; ++i)
        if (list[i].depth < 0)
          throw new ArgumentException(
            "Invalid depth value for item at index " + i + ". Only the first item (the root) should have depth below 0."
          );
    ...
    public static void UpdateDepthValues<T>(T root)
      where T : TreeElement
    {
      if (root == null)
    ...
    static bool IsChildOf<T>(T child, IList<T> elements)
      where T : TreeElement
    {
      while (child != null)
      {
    ...
    public static IList<T> FindCommonAncestorsWithinList<T>(IList<T> elements)
      where T : TreeElement
    {
      if (elements.Count == 1)
