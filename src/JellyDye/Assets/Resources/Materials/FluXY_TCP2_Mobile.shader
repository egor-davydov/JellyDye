Shader "Custom/FluXY_TCP2_Mobile" 
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "Assets/Plugins/JMO Assets/Toony Colors Pro/Shaders PBS/TCP2_PBS_Main.cginc"
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(VertexOutputForwardAdd i) : SV_Target
			{
				float2 uv = TileToUV(i.tex, _TileIndex);
				half4 state = tex2D(_MainTex, uv);
...
#else
	half4 c = TCP2_BRDF_PBS(s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusReflectivity, s.normalWorld, -s.eyeVec, light, noIndirect,	
		/* TCP2 Params */	_RampThreshold, _RampSmoothAdd, _HColor, _SColor, _SpecSmooth, _SpecBlend, fixed3(_RimMin, _RimMax, _RimStrength), 1) * state;
#endif
